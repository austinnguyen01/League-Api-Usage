...
function onMessage(msg)
{
    ...
    else if(m.type == 'addJackpotBet')
    {
        addJackpotBet(m.userInfo, m.itemsInfo);
    }
    else if(m.type == 'jackpotSet')
    {
        $('.pot-items-value').text(m.potValue);
        if(m.totalUsers == 2)
        {
            var secunde = m.timpul;
            if(secunde == 0) return;
            intervalSecunde = setInterval(function() {
                secunde -= 1;
                if(secunde == 0)
                {
                    clearInterval(intervalSecunde);
                    console.log(secunde);
                }
                $('#round-time').text(secunde);
            }, 1000);
        }
        $('.pot-value').text(m.potItemsValue.toFixed(2));
    }
    else if(m.type == 'standingQueue')
    {
        $('.client-queue').html('<i class="fa fa-user" style="color: lightgreen;"></i><p class="client-queuemsg">You are in.</p>');
        $('.client-items').text(m.totalItems);
        $('.client-chance').text(parseFloat(m.yourChance).toFixed(2));
    }
}
...
{
    $.ajax({
        url: 'localhost:3030/coinflip/sendTrade?tid='+tid,
